...
//Connect to server
var io = io("http://128.199.220.103:443/");
//Width and height of world
var ww = 3000;
var wh = 3000;
//Initial player object
var player = {id:0,x:ww/2,y:wh/2,points:30,score:0};
//Players array
var players = [];
...
var cx = function(n) {
	return (n-player.x)*cs+w/2-classes[cls].width*cs/2+shakeX;
};

//Function: return value compensating for player position (y)
var cy = function(n) {
	return (n-player.y)*cs+h/2-classes[cls].height*cs/2+shakeY;
};
...
var renderAst = function(o) {
	...
	//Draw bar
	drawLine(cx(o.x)-p,cy(o.y)+o.r+30*cs,cx(o.x)+p,cy(o.y)+o.r+30*cs,color,slw*1.2,"round");
};
...
var renderPlayer = function(o) {
	...
	drawLine(cx(o.x)+classes[c].width/2-p,cy(o.y)+classes[c].height+30*cs,cx(o.x)+classes[c].width/2+p,cy(o.y)+classes[c].height+30*cs,color,slw*1.2,"round");
	//Alias (name)
...
var background = function() {
	...
	for (i=0;i<(w/gridWidth/cs)+1;i++) {
		drawLine(cx(i*gridWidth+ox)-tx-(Math.floor(w/2/gridWidth)*gridWidth*cs),0,cx(i*gridWidth+ox)-tx-(Math.floor(w/2/gridWidth)*gridWidth*cs),h,"#A3A3A3",1,"butt");
	};
	for (i=0;i<(h/gridWidth/cs)+1;i++) {
		drawLine(0,cy(i*gridWidth+oy)-ty-(Math.floor(h/2/gridWidth)*gridWidth*cs),w,cy(i*gridWidth+oy)-ty-(Math.floor(h/2/gridWidth)*gridWidth*cs),"#A3A3A3",1,"butt");
	};*/
...
io.on("self",function(object) {
	player = {id:object.id,x:object.x,y:object.y,points:30,score:0};
});
