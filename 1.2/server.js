...
var inView = function(x1,y1,x2,y2) {
	if (x1 > x2-dss/2-200 && x1 < x2+dss/2+200 && y1 > y2-dss/2-200 && y1 < y2+dss/2+200) {
		return true;
	} else {
		return false;
	};
...
Player.prototype.emit = function() {
	...
	for (i in this.visibleAst) {
		...
		aa = aa.concat([aid,rtf(rooms[this.room].ast[aid].x,1),rtf(rooms[this.room].ast[aid].y,1),rtf(rooms[this.room].ast[aid].radius,1),rtf(rooms[this.room].ast[aid].hp/rooms[this.room].ast[aid].maxHealth,2)]);
	};
