...
var resize = function() {
	...
	menuContainer.style.transform = "translateY(-50%) scale("+cs/1.3+")";
	bottomAd.style.transform = "translateX(-50%) scale("+cs/1.3+")";
};
...
var shapes = [
	...
	function(width,height,c2) {
		...
		game.fill();
		drawLine(gamewidth/4,-height/2,width/2,height/2,c2,slw,"round");
		drawCurve(gamewidth/2,height/2,0,height/2,width/4,height/4,c2,slw,"round");
		drawCurve(game0,height/2,-width/2,height/2,-width/4,height/4,c2,slw,"round");
		drawLine(game-width/2,height/2,-width/4,-height/2,c2,slw,"round");
		drawCurve(game-width/4,-height/2,width/4,-height/2,0,-height/1.5,c2,slw,"round");
	}
];
	...
	for (i=0;i<servers.length;i++) {
		...
		x.onreadystatechange = function() {
			if (!serverFound && this.readyState == 4 && this.status == 200) {
				if (x.responseText == "1") {	
					u = x.responseURL.split("/")[2];
					serverFound = 1;
					socket = io.connect("http://"+u+":3001/",{transports: ["websocket"], upgrade: false});
					setupNetwork();
					serverLast = Date.now();
				};
			};
		};
		x.open("GET","http://"+servers[i]+"/ping",true);
		x.send();
	};
};
