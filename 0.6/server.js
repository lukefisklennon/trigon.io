...
var shake = function(x,y) {
	a = getVisible(x,y,1000,true);
	for (k in a) {
		players[a[k]].io.emit("shake");
	};
};
...
var getVisible = function(x,y,d,c) {
	...
	for (pid in players) {
		...
		if (!c || players[pid].alive) {
			...
			if (inView(x,y,players[pid].x,players[pid].y)) {//dfp(x,y,players[pid].x,players[pid].y) < d) {
				//Add to temporary a
				a.push(pid);
			};
		};
...
var getLeaderboard = function() {
	...
	for (pid in players) {
		...
		if (players[pid].alive) {
			//Add to array
			a1.push([pid,players[pid].name,players[pid].score]);
		};
	};
...
Player.prototype.die = function(id) {
	...
	if (id == this.id) {
		...
		nt = players[id].name+" destroyed "+this.name;
	};
	io.emit("notify",nt);
	this.io.emit("die");
	emitLeaderboard();
...
Player.prototype.disconnect = function() {
	...
	emitLeaderboard();
	for (pid in players) {
		players[pid].visiblePlayers.splice(players[pid].visiblePlayers.indexOf(String(this.id)),1);
	};
...
Player.prototype.update = function() {
	...
	for (j in bullets) {
		if (this.id != bullets[j].owner || (Date.now()-bullets[j].created) > 500) {
			...
			//If colliding...
			if (checkCollision(this.x+this.stats.width/2,this.y+this.stats.height/2,this.radius+20,bullets[j].x,bullets[j].y,10)) {
				//Push self
...
Ast.prototype.die = function(c) {
	...
	for (pid in players) {
		players[pid].visibleAst.splice(players[pid].visibleAst.indexOf(String(this.id)),1);
	};
	//Give points
	players[c].addPoints(this.radius/50);
	shake(this.x,this.y);
	//Delete self
	delete ast[this.id];
};
