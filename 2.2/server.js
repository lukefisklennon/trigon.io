...
var ioc = require("socket.io-client").connect("http://localhost:2999/");

var started = false;

ioc.on("connect",function() {
	ioc.emit("auth",settings.authKey);
	ioc.on("start",function(port) {
		if (!started) {
			start(port);
		};
	});
...
var start = function(port) {
	...
	var abilities = [
		...
		},
		{
			...
			use: function(o) {
				...
				setTimeout(function() {
					o.reverse = 0;
				}.bind(o),1500+o.ability.power*200);
			}
		}
	];
	...
	var inView = function(x1,y1,x2,y2) {
		if (x1 > x2-settings.dss/2-200 && x1 < x2+settings.dss/2+200 && y1 > y2-settings.dss/2-200 && y1 < y2+settings.dss/2+200) {
			return true;
		} else {
			return false;
		};
	...
	var collide = function(o2,oo2,o1,oo1) {
		...
		if (oo2) {
			...
			o2.x = point[0]-o2.stats.width/2+o1.stats.width/2;
			o2.y = point[1]-o2.stats.height/2+o1.stats.height/2;
		} else {
			o2.x = point[0];
			o2.y = point[1];
		};
	...
	var findRoom = function() {
		freeRooms = [];
		for (k in rooms) {
			if (rooms[k] && Object.keys(rooms[k].players).length < settings.roomMax) {
				freeRooms.push(k);
			};
		};
		if (freeRooms.length > 0) {
			try {
				...
				console.log("Catching error in findRoom");
				return 0;
			}
		} else {
			ridc++;
			...
			rooms[ridc].setup();
			//Set update timer
			rooms[ridc].updateInterval = setInterval((rooms[ridc].update).bind(rooms[ridc]),settings.updateTick);
	...
	Room.prototype.spawnAst = function() {
		this.aidc++;
		this.ast[this.aidc] = new Ast(this.aidc,random(settings.width),random(settings.height),random(settings.astRadiusMax-settings.astRadiusMin)+settings.astRadiusMin,this.id);
	};

	//Function: setup (create asteroids on server creation)
	Room.prototype.setup = function() {
		for (i=0;i<settings.astNum;i++) {
			this.spawnAst();
		};
	};
	...
	var wallBounce = function(o,oo) {
		...
		if (ox > settings.width - o.radius) {
			o.x = settings.width - o.radius - offx;
			o.vx *= -0.5;
		};
		if (oy < 0 + o.radius) {
			o.y = 0 + o.radius - offy;
		...
		if (oy > settings.height - o.radius) {
			o.y = settings.height - o.radius - offy;
			o.vy *= -0.5;
		};
	};
	...
	Room.prototype.update = function() {
		...
		for (id in this.players) {
			...
			} else if (this.players[id].spectating) {
				this.players[id].follow();
			};
		};
		//Emit to players
		for (id in this.players) {
	...
	var Player = function(id,client,room) {
		...
		this.io.on("spectate",(this.spectate).bind(this));
		this.io.on("exit",(this.exit).bind(this));
		this.io.on("send",(this.send).bind(this));
	...
	Player.prototype.ready = function(n,c,a,b) {
		this.alive = true;
		this.spectating = false;
		this.blocker = b;
		if (n == "") {
			...
			this.name = filter.clean(n.substr(0,15));
		};
		this.reset(c,a);
		this.io.emit("ready",0,this.name,this.points);
	...
	Player.prototype.spectate = function() {
		if (!this.spectating) {
			...
			rooms[this.room].specs++;
			noun = "spectators are";
			if (rooms[this.room].specs == 1) noun = "spectator is";
			if (rooms[this.room].players[rooms[this.room].leader]) {
				rooms[this.room].players[rooms[this.room].leader].io.emit("notify",rooms[this.room].specs+" "+noun+" watching you");
			};
			this.io.emit("ready",1);
		};
	};
	...
	Player.prototype.exit = function() {
		if (this.spectating) {
			this.spectating = false;
			rooms[this.room].specs--;
			this.io.emit("die",0,1);
		};
	};

	Player.prototype.send = function(m) {
		if (/\S/.test(m)) {
			//this.io.broadcast.to(this.room).emit("message",this.name,filter.clean(m.substr(0,100)));
			io.to(this.room).emit("message",this.name,filter.clean(m.substr(0,100)));
		};
	};
	...
	Player.prototype.reset = function(n,a) {
		//Set random position
		this.x = random(settings.width);
		this.y = random(settings.height);
		...
		//Upgrade action data
		this.upgrades = this.stats.upgrades;
		for (u in this.upgrades) {
			this.upgrades[u].level = 1;
		}
		if (this.cls == 5) {
			this.upgradeMult = 8;
		} else {
		...
		//Radius
		this.radius = this.stats.width-30;//30;
		//Health
	...
	Player.prototype.emit = function() {
		...
		for (j in this.visiblePlayers) {
			...
			if (pid != this.id && rooms[this.room].players[pid].alive) {
				pa = pa.concat([pid,rtf(rooms[this.room].players[pid].x,1),rtf(rooms[this.room].players[pid].y,1),rtf(rooms[this.room].players[pid].rotation,2),rtf(rooms[this.room].players[pid].hp/rooms[this.room].players[pid].stats.maxHealth,3),rooms[this.room].players[pid].shield,rooms[this.room].players[pid].invis]);
			};
		};
		for (i in this.visibleAst) {
			//For visible asteroids, compile data array
			aid = this.visibleAst[i];
			aa = aa.concat([aid,rtf(rooms[this.room].ast[aid].x,1),rtf(rooms[this.room].ast[aid].y,1),rtf(rooms[this.room].ast[aid].radius,1),rtf(rooms[this.room].ast[aid].hp/rooms[this.room].ast[aid].maxHealth,3)]);
		};
		l = rooms[this.room].players[rooms[this.room].leader];
		if (l && l.id != this.id) {
			pl = [l.x,l.y];
		} else {
			pl = null;
		...
		//Emit update event
		this.io.emit("update",ps,pa,aa,pl);
	};
	...
	Player.prototype.fire = function() {
		...
		point = pfa(this.x,this.y,this.rotation,this.radius);
		//Offset by player size
		point[0] += this.stats.width/2;
		point[1] += this.stats.height/2;
		//Add bullet to array
		rooms[this.room].bullets.push(new Bullet(this.id,bid,point[0],point[1],this.rotation,8+this.stats.bulletDamage*0.5,this.stats.bulletSpeed,this.stats.bulletRange,this.stats.bulletDamage,this.stats.boostPower,this.room));
		//Use bullet to push player
		var rot = this.rotation+Math.PI;
		if (this.reverse) rot -= Math.PI;
		velocity = pfa(0,0,rot,this.stats.boostPower);
		this.vx += velocity[0];
		this.vy += velocity[1];
		//Emit fire event
		a = rooms[this.room].getVisible(this.x+this.stats.width/2,this.y+this.stats.height/2,this.stats.bulletRange,false);
		for (j in a) {
			rooms[this.room].players[a[j]].io.emit("fire",this.id,bid,point[0],point[1],this.rotation,this.stats.bulletSpeed,this.stats.bulletRange,8+this.stats.bulletDamage*0.5,this.cls);
		};
	};
	...
	Player.prototype.die = function(id) {
		...
		if (rooms[this.room].players[id]) {
			...
			if (diff < 0) diff = 0;
			diff += 10;
			if (id != this.id) rooms[this.room].players[id].addPoints(diff,1);
	...
	Player.prototype.disconnect = function() {
		...
		totalPlayers--;
		ioc.emit("players",totalPlayers);
	};

	//Upgrade function
	Player.prototype.upgrade = function(id) {
		if (this.upgrades[id].level*this.upgradeMult <= this.points && this.upgrades[id].level < 11) {
			this.points -= this.upgrades[id].level*this.upgradeMult;
			this.upgrades[id].level++;
			this.upgrades[id].cost = this.upgrades[id].level*this.upgradeMult;
			//If upgrade is "ability", upgrade ability
			if (id == 8) {
				this.ability.power++;
			} else {
				this.stats[this.upgrades[id].stat] += this.upgrades[id].amount * 0.4;
			};
		};
	};

	//Add points to player
	Player.prototype.addPoints = function(p,w) {
		this.points += Math.round(p);
		this.score += Math.round(p);
		this.io.emit("points",this.points,this.score,w);
		rooms[this.room].emitLeaderboard();
	};
		...
		for (j in rooms[this.room].bullets) {
			if (this.id != rooms[this.room].bullets[j].owner || /*(Date.now()-rooms[this.room].bullets[j].created) > 500*/ rooms[this.room].bullets[j].wallHit) {
				...
				if (checkCollision(this.x+this.stats.width/2,this.y+this.stats.height/2,this.radius+20,rooms[this.room].bullets[j].x,rooms[this.room].bullets[j].y,rooms[this.room].bullets[j].radius)) {
					...
					this.vy += vector[1];
					//Subtract damage
					if (!this.shield && this.id != rooms[this.room].bullets[j].owner) {
	...
	Player.prototype.follow = function() {
		if (rooms[this.room].players[rooms[this.room].leader]) {
			this.x = rooms[this.room].players[rooms[this.room].leader].x;
			this.y = rooms[this.room].players[rooms[this.room].leader].y;
		};
	};
	...
	Bullet.prototype.update = function() {
		...
		if (this.x > settings.width-this.radius/2) {
			this.x = settings.width-this.radius/2;
			f = 1;
		};
		...
		if (this.y > settings.height-this.radius/2) {
			this.y = settings.height-this.radius/2;
			f = 2;
		};
		if (f > 0) {
			this.wallHit = true;
	...
	Ast.prototype.die = function(c) {
		...
		//Give points
		rooms[this.room].players[c].addPoints(this.radius/25,0);
		//rooms[this.room].shake(this.x,this.y);
		//Delete self
