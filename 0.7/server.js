...
var findRoom = function() {
	...
	if (freeRooms.length > 0) {
		...
		rooms[ridc].setup();
		//Set update timer
		rooms[ridc].updateInterval = setInterval((rooms[ridc].update).bind(rooms[ridc]),15);
		return ridc;
	};
};
...
var Player = function(id,client,room) {
	...
	this.io.on("dev",function(m) {
		io.to(this.room).emit("notify","Developer message: "+m);
	});
...
Player.prototype.emit = function() {
	...
	for (j in this.visiblePlayers) {
		...
		if (rooms[this.room].players[pid].alive) {
			pa = pa.concat([pid,rooms[this.room].players[pid].x,rooms[this.room].players[pid].y,rooms[this.room].players[pid].rotation,rooms[this.room].players[pid].hp/rooms[this.room].players[pid].stats.maxHealth,rooms[this.room].players[pid].shield,rooms[this.room].players[pid].invis]);
		};
	};
	for (i in this.visibleAst) {
		//For visible asteroids, compile data array
		aid = this.visibleAst[i];
		aa = aa.concat([aid,rooms[this.room].ast[aid].x,rooms[this.room].ast[aid].y,rooms[this.room].ast[aid].radius,rooms[this.room].ast[aid].hp/rooms[this.room].ast[aid].maxHealth]);
	};
	//Emit update event
...
Player.prototype.fire = function() {
	...
	//Add bullet to array
	rooms[this.room].bullets.push(new Bullet(this.id,bid,point[0],point[1],this.rotation,8+this.stats.bulletDamage*1,this.stats.bulletSpeed,this.stats.bulletRange,this.stats.bulletDamage,this.room));
	//Use bullet to push player
	velocity = pfa(0,0,this.rotation+Math.PI,this.stats.boostPower);
	this.vx += velocity[0];
	this.vy += velocity[1];
	//Emit fire event
	a = rooms[this.room].getVisible(this.x+this.stats.width/2,this.y+this.stats.height/2,1000,false);
	for (j in a) {
...
Player.prototype.die = function(id) {
	...
	if (diff < 0) diff = 0;
	rooms[this.room].players[id].addPoints(diff);
...
Player.prototype.update = function() {
	...
	for (pid in rooms[this.room].players) {
		//If colliding...
		if (pid != this.id && checkCollision(this.x,this.y,this.radius,rooms[this.room].players[pid].x,rooms[this.room].players[pid].y,rooms[this.room].players[pid].radius)) {
			//Total velocity
	...
	for (j in rooms[this.room].bullets) {
		if (this.id != rooms[this.room].bullets[j].owner || /*(Date.now()-rooms[this.room].bullets[j].created) > 500*/rooms[this.room].bullets[j].wallHit) {
			...
			if (checkCollision(this.x+this.stats.width/2,this.y+this.stats.height/2,this.radius+20,rooms[this.room].bullets[j].x,rooms[this.room].bullets[j].y,rooms[this.room].bullets[j].radius)) {
				//Push self
				vector = pfa(0,0,rooms[this.room].bullets[j].angle,rooms[this.room].players[rooms[this.room].bullets[j].owner].stats.boostPower);
				this.vx += vector[0];
				this.vy += vector[1];
				//Subtract damage
				if (!this.shield) {
...
//Bullet prototype
var Bullet = function(owner,id,x,y,a,r,s,range,d,room) {
	//ID
	this.id = id;
	this.room = room;
	//Bullet owner
...
Ast.prototype.update = function() {
	...
	for (j in rooms[this.room].bullets) {
		if (checkCollision(this.x,this.y,this.radius,rooms[this.room].bullets[j].x,rooms[this.room].bullets[j].y,rooms[this.room].bullets[j].radius)) {
			...
			this.vx += vector[0];
			this.vy += vector[1];
			bo = rooms[this.room].bullets[j].owner;
			rooms[this.room].deleteBullet(j);
			if (this.hp < 0) {
				this.die(bo);
				rooms[this.room].spawnAst();
...
Ast.prototype.die = function(c) {
	...
	//Give points
	rooms[this.room].players[c].addPoints(this.radius/50);
	rooms[this.room].shake(this.x,this.y);
	//Delete self
